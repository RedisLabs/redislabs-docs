...
- Any float that is Not a Number will return `nan` ([#11597](https://github.com/redis/redis/pull/11012)).
...
- When a blocked client is being unblocked, checks for ACLs and OOM condition checks are now re-evaluated ([#11012](https://github.com/redis/redis/pull/11012)).
    - If the ACL rules have changed since the command was executed, the command might fail once the client is unblocked.
