...
1. In the Node Configuration page that appears:
    ...
    1. Enter a path for [**Ephemeral storage**]({{< relref "/rs/installing-upgrading/install/plan-deployment/persistent-ephemeral-storage" >}}),
        or leave the default path.

    1. If you want to enable Redis on Flash, select the **Enable flash storage support** checkbox
        and enter the path to the [Flash storage]({{< relref "/rs/databases/auto-tiering/" >}}).
