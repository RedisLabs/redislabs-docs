...
| bdb_crdt_syncer_ingress_bytes | Rate of compressed incoming network traffic to Active-Active (bytes/sec) |
| bdb_crdt_syncer_ingress_bytes_decompressed | Rate of decompressed incoming network traffic to Active-Active (bytes/sec) |
| bdb_crdt_syncer_local_ingress_lag_time | Lag time between the source and the destination (ms) for Active-Active traffic |
| bdb_crdt_syncer_status | Syncer status for Active-Active traffic; 0 = in-sync, 1 = syncing, 2 = out of sync |
