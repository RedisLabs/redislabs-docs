---
Title: Create an Active-Active Replicated Database (CRDB)
description:
weight: $weight
alwaysopen: false
categories: ["RS"]
---
[Active-Active databases]({{< relref "/rs/administering/active-active.md" >}}) (also known as Conflict-free Replicated DataBases or CRDBs) let you create replicated instances of your data between Redis Enterprise Software (RS) clusters.
...
Every instance of an Active-Active database can receive write operations, and all operations are [synchronized]({{< relref "/rs/concepts/intercluster-replication.md#example-of-synchronization" >}}) to all of the instances.
...
- You cannot enable or disable database clustering after the Active-Active database is created.
    ...
    - **Replication** - We recommend that you use intra-cluster replication to create slave shards in each Active-Active database instance.
    ...
    - **Redis password** - A password that clients must use to connect to the Active-Active database.
...
<!-- Also in crdbs.md -->
    ...
    - **Participating Clusters** - You must specify the URL of the clusters that you want to
        host Active-Active database instances and the admin user account to connect to each cluster.
    ...
    - **Causal Consistency** - Causal Consistency in an Active-Active database guarantees that the order of operations on a
        specific key is maintained across all Active-Active database instances. To enable Causal Consistency for an existing
    ...
    - **TLS** - You can enable TLS for communications between
        ...
        cluster that only impacts the specific Active-Active database instance you are editing and
        does not apply automatically to all Active-Active database instances.

<!-- Also in getting-started-crdbs.md -->
## Test the Connection to your Member Redis Active-Active Database
