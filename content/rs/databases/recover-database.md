...
1. [Restore the cluster configuration]({{< relref "/rs/clusters/recover/cluster-recovery.md" >}}) from the CCS files
...
1. If the cluster failed, [recover the cluster]({{< relref "/rs/clusters/recover/cluster-recovery.md" >}}).
...
- Before you start database recovery, make sure that the cluster that hosts the database is healthy.
    ...
    you must [recover the cluster]({{< relref "/rs/clusters/recover/cluster-recovery.md" >}}) before you recover the databases.
