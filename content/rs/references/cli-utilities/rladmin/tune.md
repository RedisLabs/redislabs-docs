...
| db_conns_auditing                      | `enabled`<br /> `disabled`      | Activates or deactivates database [connection auditing]({{<relref "/rs/security/audit-events">}}) for a cluster                                                                 |
...
| max_simultaneous_backups               | integer                           | Number of database backups allowed to happen at once. Combined with tune node max_redis_forks will determine the parallel backup for shards                                                                                  |
