...
|`replication true`| boolean| Enables [database replication]({{< relref "/rs/databases/replication.md" >}})| where every master shard replicates to a replica shard (We recommend that you use replication so that active-active database synchronization traffic is off-loaded to the slave shard)|
|`encryption true`| boolean| Enable encryption|
|`sharding disable`| string| Disable sharding (also known as [database clustering]({{< relref "/rs/databases/replication.md" >}})) so that there is only one shard for the database|
