...
Do not upgrade a cluster that hosts Active-Active databases with modules before performing the following steps:

1. Check the module and its version as it appears in the database configuration and in the default database configuration using `crdb-cli`. The fields are called `modules` and `module_list`.

    ```sh
    crdb-cli crdb get --crdb-guid <crdb-guid>
    ```

1. From the `redis modules` tab of the Redis Enterprise Software admin console, validate that these modules with their specific versions are loaded to the cluster.

1. In case one or more of the modules+versions are missing or if you need help, [contact Redis support](https://redis.com/company/support/) before taking additional steps.
