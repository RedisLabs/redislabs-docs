...
RS 5.6.0 adds support for [Redis on Flash]({{< relref "/rs/databases/redis-on-flash/" >}}) (RoF) databases with [Redis Modules](https://docs.redis.com/latest/modules). [RedisJSON](https://docs.redis.com/latest/modules/redisjson) is the first module to be available to run on a Redis on Flash database.
