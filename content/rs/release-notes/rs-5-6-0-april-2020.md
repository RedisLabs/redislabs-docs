...
RS 5.6.0 adds the support of [HyperLogLog](http://www.antirez.com/news/75) data structure for [Active-Active Redis]({{< relref "/rs/databases/replica-of.md" >}}) Databases.
...
- RS51359 - Active-Active databases, using replication and Append Only File (AOF) for [Database Persistence]({{< relref "/rs/concepts/memory-performance/persistence.md" >}}), are suffering from memory leaks on replica shards, causing them to grow bigger than the master shards. Customers are advised to upgrade to RS 6.0.12. Meanwhile, you can use a snapshot for Database Persistence or restart the replica shards.
