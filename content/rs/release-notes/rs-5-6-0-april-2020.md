...
RS 5.6.0 adds support for [Redis on Flash]({{< relref "/rs/databases/redis-on-flash/.md" >}}) (RoF) databases with [Redis Modules]({{< relref "/modules/_index.md" >}}). [RedisJSON]({{< relref "/modules/redisjson/_index.md" >}}) is the first module to be available to run on a Redis on Flash database.
...
For more information, check out the [OSS Cluster API documentation]({{< relref "/rs/databases/configure/enable-oss-cluster-api.md" >}}).
