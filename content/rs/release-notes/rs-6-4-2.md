...
Redis Enterprise Software version 6.2.x packages two Redis DB versions: Redis DB 6.0 and Redis DB 6.2. Until now, the default Redis DB version for creating new databases and upgrading existing databases was 6.0 (enforced by the `upgrade_policy` parameter).

To allow customers more flexibility in future upgrades, starting with Redis Enterprise Software 6.4.2, the default Redis DB version for new and upgraded databases is now 6.2 for both `upgrade_policy=major` and `upgrade_policy=latest`. 
...
To run on Amazon Linux 2, Redis Enterprise Software uses the same package as the one used for RedHat Enterprise Linux (RHEL) 7. The package uses a version of the “Boost” library that may be incompatible with newer machines. This limitation will be fixed in the next maintenance release. 
...
Redis Enterprise Software v6.2 end of life (EOL) is August 31, 2023.
...
For information and lifecycles of Redis modules, see [Module lifecycle](https://docs.redis.com/latest/modules/modules-lifecycle/).
