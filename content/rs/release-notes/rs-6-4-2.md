...
Do not upgrade a cluster that hosts Active-Active databases with module/s before following the next steps:
Check the module and its version as it appears in the db config and in the default db config using the `crdb-cli crdb get --crdb-guid <crdb-guid>`. The fields are called 'modules' and 'module_list'.
Via the UI Admin console, under the 'redis modules' tab, validate that these module/s with their specific version/s are loaded to the cluster.
In case one or more of the modules+versions are missing, or if you need help [contact Redis support](https://redis.com/company/support/) before taking additional steps.
This limitation has been fixed and resolved starting v6.4.2-43.
