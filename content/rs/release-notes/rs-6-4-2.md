...
####Reshard fails for Rack-Aware Databases with no replication
When database is configured as [rack-aware]({{<relref "/rs/clusters/configure/rack-zone-awareness">}}) and replication is disabled, reshard operation will fail.
RS97971 - Fix will be released as part of April maintenance release                          
Workaround:
To perform reshard you should first disable rack-aware, following reshard you may enable it again
```sh
...
Before you upgrade a cluster that hosts Active-Active databases with modules, perform the following steps:
