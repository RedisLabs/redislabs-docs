...
(Please read the below updates for 6.0.20-95)
...
-   The bundled RedisSearch module was upgraded to v2.0.9
...
-   RS57315 - Fixed creation and editing an Active-Active database with LDAP users of the new RBAC LDAP integration. This applies to UI access and REST API and does not apply to LDAP users for database access via Redis Clients
-   RS57073 - Fixed a bug casued in the shard migration process which could leave unattended shards on the node
-   RS56508 - Fixed backwards compatibiliy of client certificate when upgrading from ealier versions and using a certificate chain with Extended Key Usage extension being set to “TLS Web Server Authentication” instead of “TLS Web Client Authentication”
