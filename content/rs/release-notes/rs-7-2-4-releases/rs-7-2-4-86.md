...
Redis Enterprise Software version 7.2.4-86 includes the following Redis Stack modules:
...
- RS114185 - Fixed an issue where during an upgrade to the initial release of Redis Enterprise Software version 7.2.4-86, the proxy might not start due to a `Failed to get default_suffix` error, which appears in `dmcproxy.log`.
