...
The cluster admin user password is created by the Operator during the deployment of the REC and is stored in a Kubernetes secret. At this time, this password cannot be changed. 
Do not change this password throught the Redis Enterprise UI. Changes of the admin password will impact the operation of the K8s deployment.
You can create additional users with admin priviledges and control their passwords if needed.
{{% /expand%}}
...
>Note: The default admin user password is automatically generated by the Operator and cannot be changed at this time. Do not change this password in the Redis Enterprise UI.
...
{{%expand "What capabilities, privileges and permissions are defined by the Security Context Constraint (SCC) yaml? Pod Security Policy (PSP) yaml?" %}}
...
While the RLEC container runs as user 1001, there are no limits currently set on users and user groups in the default scc.yaml file while the psp.yaml example defines the specific uid.
...
RLEC PSP definitions are controlled through RBAC - a cluster role allowing the RLEC PSP is granted the to redis-enterprise-operator service account, allowing that account to create pods with the above mentioned PSP.
