...
### [Swagger UI](https://api-beta1-qa.redislabs.com/beta1/swagger-ui.html)

Useful for initial introduction and learning about API operations, models and simulated usage via a web based user interface
...
**Important Note:** the code generated by Swagger Codegen (or a custom code wrapping the HTTP API calls) is not supported but Redis Labs ltd. Redis Labs API supports only the HTTP layer of communication between a standard HTTP request and the API. If any issues are encountered while interacting with the API, the raw HTTP call (URI, body and parameters) will be required by Redis Labs ltd. personnel in order to provide support and assistance.
