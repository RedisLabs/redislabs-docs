...
({{< relref "/rv/api/concepts/provisioning-lifecycle.md#asynchronous-operations" >}}).
...
- `status` - The [status]({{< relref "/rv/api/concepts/provisioning-lifecycle.md#provisioning-statuses">}}) of the task
...
- The `links` array contains another `getSubscriptionInformation` URL that links to the newly created subscription.
    This link queries the subscription status during [provisioning]({{< relref "/rv/api/concepts/provisioning-lifecycle.md" >}}))
