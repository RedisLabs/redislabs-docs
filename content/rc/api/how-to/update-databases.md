...
You can see [the complete set of JSON elements]({{< relref "/rc/api/how-to/using-curl.md#inputs-for-operations-in-swagger" >}}) accepted by the database update API operation in the [Swagger UI](https://api.redislabs.com/v1/swagger-ui.html) (expand the specific API operations and select `Model` in the `request` section).
