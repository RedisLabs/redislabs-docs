...
3. From here, you need to:
    ...
    1. Set up the deployment options for your subscription, include cloud vendor details, high avaliability settings, and advanced options.
...
- [Advanced options](#advanced-options) define settings for high avaliability and security. These will vary according to cloud provider.
...
|:---------|:-----------|
| **Multi-AZ** | Determines if high avaliability spans multiple Availablity Zones, which provides automatic failover when problems occur. |
...
| **High Avaliability** | Indicates whether a replica copy of the database is maintained in case the primary database becomes unavailable.  (Warning: Doubles memory consumption). |
...
| **Type** | Set to _Memcached_ database to support the legacy database; otherwise leave at _Redis_ |
