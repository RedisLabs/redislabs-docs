...
3.  Choose a **Cloud Provider** and a **Region**.
    ...
    A preview of Redis 7.0 subscriptions is available for selected regions in AWS and Google Cloud. However, some Redis 7.0 functionality might not be fully available during preview. Redis 7.0 also introduces several changes to existing Redis commands; see the list of [breaking changes]({{<relref "/rs/references/compatibility/breaking-changes">}}) for more details.
