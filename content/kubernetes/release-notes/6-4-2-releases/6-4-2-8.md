...
{{<expand "Following the previous quick start guide version causes issues with creating an REDB due to unrecognized memory field name (RED-69515)">}} The workaround is to use the newer (current) revision of the [quick start]({{<relref "kubernetes/deployment/quick-start">}}). {{</expand>}}
