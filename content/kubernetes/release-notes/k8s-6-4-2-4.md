...
* Resharding fails for rack-aware databases with no replication (RED-97971)
  ...
  Workaround:
    Before resharding your database, [turn off rack awareness]({{<relref "/kubernetes/reference/db-options/">}}). Set the `spec.rackAware` field in your REDB custom resource to `false`.
    After the resharding process is complete, you can [re-enable rack awareness]({{<relref "/kubernetes/reference/db-options/">}}).
