...
* Allowed nginx ingress class annotation to contain "`ngnix`" as a substring instead of being exactly "`ngnix`" (RED-79205).
...
[Redis Enterprise cluster (REC)](https://github.com/RedisLabs/redis-enterprise-k8s-docs/blob/master/redis_enterprise_cluster_api.md):

* Added `APIServiceSpec` to allow configuration of the API service type.
* Made `redisOnFlash` available by default.
* Made `ocsp` available by default for configuring OCSP stapling.
