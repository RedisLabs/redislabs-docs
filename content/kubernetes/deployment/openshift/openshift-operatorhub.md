...
1. Edit the [custom resource definition]({{< relref "/kubernetes/re-clusters/cluster-options.md">}}).

    {{< note >}}
The name of the cluster must be "rec" for deployments from within the OLM. For more information, see the `name:` field description in [**Options for clusters**]({{< relref "/kubernetes/re-clusters/cluster-options.md">}}).
    {{< /note >}}
