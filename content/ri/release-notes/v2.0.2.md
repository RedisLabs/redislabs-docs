...
* **Support for Apple M1 (arm64)** - You can downaload it [here](https://redis.com/redis-enterprise/redis-insight/#insight-form).
* **Added auto-discovery of local databases** - RedisInsight will automatically find and add your local databases when you open the application for the very first time.
* **A dedicated Editor for Cypher syntax** - Use Workbench to work with your RedisGraph queries and enjoy autocomplete and highlights of the Cypher syntax. 

## Details: 
- You can switch to the Tree view in Browser to see all the keys grouped into folders according to their namespaces. Note that we use the colon (:) as a default separator, and it is not customisable yet.
...
Please note that if you installed RedisInsight-preview before, this folder will still exist at the following path:
