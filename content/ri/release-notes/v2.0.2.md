...
Enjoy a new Tree view available in Browser, which automatically groups keys scanned in your database into folders based on key namespaces. Now you can navigate through and analyze your list of keys quicker by opening only folders with namespaces you want.

### Support for Apple M1 (arm64)

This release introduces the support for Apple M1 (arm64). You can downaload it [here](https://redis.com/redis-enterprise/redis-insight/#insight-form).

### Added auto-discovery of local databases

Now RedisInsight will automatically find and add your local databases when you open the application for the very first time.

### Introduced a dedicated Editor for Cypher syntax

Use Workbench to work with your RedisGraph queries and enjoy autocomplete and highlights of the Cypher syntax. 

## Details: 
- Now you can switch to the Tree view in Browser to see all the keys grouped into folders according to their namespaces. Note that we use the colon (:) as a default separator, and it is not customisable yet.
...
- Added tutorials in Workbench for Redis Stack database that describe a common use case through Redis capabilities.
